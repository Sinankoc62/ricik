...
// ==========================================
// API ROTASI: GİRİŞ (LOGIN - GEÇİCİ TEST MODLU)
// ==========================================
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    // 🛠️ Geliştirme aşaması için GEÇİCİ TEST KAPISI:
    // Eğer girilen şifre '123456' ise hash kontrolünü es geçip onay veriyoruz.
    let gecen = false;
    if (sifre === '123456') {
      gecen = true; 
    } else {
      gecen = await bcrypt.compare(sifre, user.sifre_hash);
    }
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    if (sifre === '123456') {
      console.log('TEST PASSWORD USED');
      gecen = true;
    } else {
      console.log('BCRYPT START');

      gecen = await bcrypt.compare(
        sifre,
        user.sifre_hash
      );

      console.log('BCRYPT RESULT:', gecen);
    }
    ...
    if (user.durum === 'onay_bekliyor') {
      ...
      console.log('USER WAITING APPROVAL');
